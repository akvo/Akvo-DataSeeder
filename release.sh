...
LIB_NAME="Akvo-DataSeeder"
CURRENT_VERSION=$(< ./src/AkvoDataSeeder/__init__.py tr ' ' _ \
    | grep __version__ \
    | cut -d "=" -f2 \
    | sed 's/"//g' \
    | sed 's/_/v/g'
...
if [[ "$CURRENT_VERSION" == "$CURRENT_TAG" ]]; then
    ...
    printf "Located at ./src/AkvoDataSeeder/__init__.py\n"
...
function push_release() {
    ...
    gh api \
        ...
        "/repos/akvo/Akvo-DataSeeder/releases" \
        -f tag_name="$1" \
